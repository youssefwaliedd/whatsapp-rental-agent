...
class WorkItem(Base):
    """Durable inbox/outbox. Payloads are cleared after successful processing."""
    __tablename__ = "work_items"
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    key: Mapped[str] = mapped_column(String, unique=True)
    lane: Mapped[str] = mapped_column(String, index=True)
    kind: Mapped[str] = mapped_column(String)
    payload: Mapped[dict] = mapped_column(JSON)
    status: Mapped[str] = mapped_column(String, default="pending", index=True)
    attempts: Mapped[int] = mapped_column(Integer, default=0)
    available_at: Mapped[datetime] = mapped_column(AwareDateTime, index=True)
...
class WorkerHeartbeat(Base):
    ...
    seen_at: Mapped[datetime] = mapped_column(AwareDateTime)


class DocumentCheck(Base):
    """Minimal automated result, without extracted identity numbers or names."""
    __tablename__ = "document_checks"
    document_id: Mapped[str] = mapped_column(ForeignKey("customer_documents.document_id"), primary_key=True)
    model: Mapped[str] = mapped_column(String)
    name_digest: Mapped[str | None] = mapped_column(String, default=None)
    issues: Mapped[list] = mapped_column(JSON)
    checked_at: Mapped[datetime] = mapped_column(AwareDateTime)


class DocumentFacts(Base):
    """Encrypted eligibility facts, separate from the minimal check result."""
...
class CustomerDocument(Base):
    """Private attachment receipt. Only a staff decision can approve it."""

    __tablename__ = "customer_documents"

    document_id: Mapped[str] = mapped_column(String, primary_key=True)
    provider_message_id: Mapped[str] = mapped_column(String, unique=True)
    ...
    reservation_id: Mapped[str | None] = mapped_column(ForeignKey("reservations.reservation_id"), default=None)
    storage_key: Mapped[str | None] = mapped_column(String, default=None)
    mime_type: Mapped[str | None] = mapped_column(String, default=None)
    sha256: Mapped[str | None] = mapped_column(String, default=None)
    size_bytes: Mapped[int] = mapped_column(Integer, default=0)
    status: Mapped[str] = mapped_column(String, default="download_failed", index=True)
    ...
    created_at: Mapped[datetime] = mapped_column(AwareDateTime)
    reviewed_at: Mapped[datetime | None] = mapped_column(AwareDateTime, default=None)
    review_copy_sent_at: Mapped[datetime | None] = mapped_column(AwareDateTime, default=None)
...
class MessagePresentation(Base):
    """Rendered chat content, separate from the model's plain-text transcript.

    An additive table also works with existing databases without altering the
    messages table or fabricating historical media that was never stored.
    """
