...
#: decimal places.
_MONEY = re.compile(
    r"(?:aed|dhs?|dirhams?|\$|usd)\s*(\d[\d,]*(?:\.\d+)?)"
    r"|(\d[\d,]*(?:\.\d+)?)\s*(?:aed|dhs?|dirhams?)"
    r"|(\d{1,3}(?:,\d{3})+(?:\.\d{1,2})?)"
    r"|(\d+\.\d{2})\b",
    re.I,
)
...
SAFE_REPLY = (
    "Let me double-check those numbers for you and come straight back — I would "
    "rather give you the exact figure than a rough one."
)
...
def inspect(reply: str, tool_calls: Iterable[Any]) -> Verdict:
    """Every figure in the reply, checked against what the tools returned.

    Skipped when nothing was called: a conversational turn with no tools has no
    evidence to check against, and treating that as a violation would block
    "good morning".
    """
    ...
    supported = supported_numbers(calls)
